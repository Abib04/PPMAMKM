...
$.fn.easyPaginate = function (options) {
    ...
    return this.each (function (instance) {        
        ...
        var displayNav = function() {
            ...
            htmlNav = '<ul class="paginationcus pagination pagination-sm" style="margin-top: 0px;margin-bottom: 0px; position:absolute ;top:40px;">';
            ...
            $(elSelector + ' .pagination a.page,'
                + elSelector + ' .pagination a.first,'
                + elSelector + ' .pagination a.last').on('click', function(e) {
                e.preventDefault();
                displayPage($(this).attr('rel'));                
            });

            $(elSelector + ' .pagination a.prev').on('click', function(e) {
                e.preventDefault();
                page = plugin.settings.currentPage > 1?parseInt(plugin.settings.currentPage) - 1:1;
                displayPage(page);
            });

            $(elSelector + ' .pagination a.next').on('click', function(e) {
                e.preventDefault();
        ...
        var displayPage = function(page, forceEffect) {
            if(plugin.settings.currentPage != page) {
                ...
                switch(plugin.settings.currentPage) {
                    case 1:
                        $('.pagination li', plugin).removeClass('disabled');
                        $('.pagination li:first, .pagination li:nth-child(2)', plugin).addClass('disabled');
                        break;
                    case plugin.settings.pages:
                        $('.pagination li', plugin).removeClass('disabled');
                        $('.pagination li:last, .pagination li:nth-last-child(2)', plugin).addClass('disabled');
                        break;
                    default:
                        $('.pagination li', plugin).removeClass('disabled');
                        break;
