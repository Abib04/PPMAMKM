...
if($_SESSION['cetak_sertif'] != 1 ){
    exit("Maaf, anda tidak bisa mengakses halaman ini.");
}

$npm = (is_npm($_SESSION['nim_ppm']))? $_SESSION['nim_ppm']:'';
...
$ser = db_read("SELECT `nama`,`npm`,`no`,`posisi`,`predikat`,`no_cert`,`tanggal_terbit` as `tanggal` FROM `vsertifikat` WHERE `npm` = '".$npm."'");
if(!count($ser) > 0)
    die("data tidak ada!!");

$data = $ser[0];
$tglacara = db_read("SELECT min(`tgl_mulai`) as mulai, MAX(`tgl_selesai`) as selesai FROM `acara_tahun` WHERE id_thn = ".$_SESSION['mhs_thn_ke'])[0];
...
$pdf->Image("resource/mahasiswa/qrcode_sertif/".$npm.".png",64,196,48,0,'PNG');

$filename = "resource/mahasiswa/foto_mhs/".$npm.".jpg";

// var_dump($filename);
// die();

if (file_exists($filename)) {
    $pdf->Image($filename,110,200.55,0,38.5,'JPG');
}
...
$pdf->SetY(140);
$pdf->MultiCell(190,6,"Penggalian Potensi Mahasiswa (PPM) ".$tahun." ",'','C',false);

$pdf->SetFont('Times','B',18);
$pdf->SetY(97);
$pdf->Cell(190,6,$data['nama'],'',0,'C','','');
$pdf->SetY(105);
$pdf->Cell(190,6,$_SESSION['nim_skrng'],'',0,'C','','');
