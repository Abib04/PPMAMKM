...
if(is_null($op)){
    ...
    if($op == "create" || $op == "update"){
        $data['id_kloter'] = $_POST['kloter'];
    }
    
    if($op == "create"){
        $data["nama_kelompok"] = cleanchar($_POST['kelompok']);
        ...
        $thn = (isset($_GET['thn'])) ? cleanchar($_GET['thn']) : NULL;
        if(!is_null($thn)){
            echo json_encode(db_read("select * from vkelompok  where id_thn=".$thn.""));
        } elseif (!is_null($id)){
            // echo $id;
