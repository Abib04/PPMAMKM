...
if(is_null($op)){
    ...
    if($op == "create" or $op == "update"){
        ...
        $data = array(
            "nama_agama" => cleanchar($_POST['nama_agama']),
            "status" => cleanchar($_POST['status_agama'])
        );
    ...
    if($op == "create"){
        ...
        if(!is_null($id)){
            echo json_encode(db_read("select * from $def_table where id_agama='$id'"));
        }else{
        ...
        if(!is_null($id)){
            ...
            if(db_update($def_table, $data, array("id_agama" => $id))){
                echo "true";
            }else{
                echo $_SESSION['err_message'];
            }
        ...
        if(is_null($id) == FALSE){
            if(db_delete($def_table, array("id_agama" => $id))){
                echo "true";
            }else{
                echo $_SESSION['err_message'];
            }
